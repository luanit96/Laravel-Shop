@extends('layouts.app')
@section('content')
<div class="inner-header">
	<div class="container">
		<div class="pull-left">
			<h6 class="inner-title">Liên hệ</h6>
		</div>
		<div class="pull-right">
			<div class="beta-breadcrumb font-large">
				<a href="{{route('home')}}">Trang chủ</a> / <span>Liên hệ</span>
			</div>
		</div>
		<div class="clearfix"></div>
	</div>
</div>
<div class="beta-map">

	<div class="abs-fullwidth beta-map wow flipInX"><iframe src="https://www.google.com/maps/embed?pb=!1m14!1m8!1m3!1d3678.0141923553406!2d89.551518!3d22.801938!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x39ff8ff8ef7ea2b7%3A0x1f1e9fc1cf4bd626!2sPranon+Pvt.+Limited!5e0!3m2!1sen!2s!4v1407828576904" ></iframe></div>
</div>
<div class="container">
	<div id="content" class="space-top-none">

		<div class="space50">&nbsp;</div>
		<div class="row">
			<div class="col-sm-8">
				<h2>Form Liên hệ</h2>
				<div class="space20">&nbsp;</div>
				<form action="{{route('lienhe')}}" method="post" class="contact-form">
					<input type="hidden" name="_token" value="{{csrf_token()}}">
					@if(count($errors)>0)
					<div class="alert alert-danger">
						@foreach($errors->all() as $err)
						{{$err}}
						@endforeach
					</div>
					@endif
					@if(Session::has('thanhcong'))
					<div class="alert alert-success">
						{{Session::get('thanhcong')}}
					</div>
					@endif	
					<div class="form-block">
						<input name="fullname" type="text" placeholder="Họ và tên">
					</div>
					<div class="form-block">
						<input name="email" type="email" placeholder="Email">
					</div>
					<div class="form-block">
						<input name="phone" type="text" placeholder="0965522247">
					</div>
					<div class="form-block">
						<textarea name="detail" placeholder="Nội dung liên hệ"></textarea>
					</div>
					<div class="form-block">
						<button type="submit" class="beta-btn primary">Gửi<i class="fa fa-chevron-right"></i></button>
					</div>
				</form>
			</div>
			<div class="col-sm-4">
				<h2>Liên hệ với chúng tôi</h2>
				<div class="space20">&nbsp;</div>

				<h6 class="contact-title">Địa chỉ cửa hàng:</h6>
				<p>
					270L Võ Thị Sáu, Phường 7, Q. 3, Tp. Hồ Chí Minh
				</p>
				<div class="space20">&nbsp;</div>
				<h6 class="contact-title">Điện thoại:</h6>
				<p>
					028 7300 2010 - 024 7300 2010 (24/7)
				</p>
				<div class="space20">&nbsp;</div>
				<h6 class="contact-title">Email:</h6>
				<p>
					<a href="hr@betadesign.com">contact@backersAlley.com</a>
				</p>
			</div>
		</div>
	</div> <!-- #content -->
</div> <!-- .container -->
@endsection